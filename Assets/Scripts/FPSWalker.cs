...
public class FPSWalker : MonoBehaviour
{
    ...
    [Tooltip("How high the player jetpacks when hitting the jump button in the air.")] [SerializeField]
    private float m_JetpackSpeed = 4.0f;
    ...
    public AudioClip hurtSound; // this one is specific to the player
    ...
    {
        if (Physics.SphereCast(transform.position + Vector3.up, 1f, Vector3.up, out m_Hit))
        {
        ...
        if (grounded)
        {
            ...
            else
            {
                ...
                // walk sound
                if (inputX > 0.01f || inputX < -0.01f || inputY > 0.01f || inputY < -0.01f)
                {
                    // you are walking, play walk sound
        ...
        {
            // jetpack in the air
            m_MoveDirection.y = m_JetpackSpeed;
    ...
    private void OnControllerColliderHit(ControllerColliderHit hit)
    {
        ...
        // {
        //     // touching something on the sides, we can wall jump
        //
        //     touchingWall = true;
    ...
    {
        if (other.gameObject.layer != LayerMask.NameToLayer("Contestant") && other.gameObject.layer != LayerMask.NameToLayer("Gun"))
        {
