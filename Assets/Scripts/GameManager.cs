...
{
    /// <summary>
    /// Gunblur is about surviving. Just keep track of # alive enemies, kills, time, etc.
    /// This script also spawns enemies.
    /// </summary>
    ...
    public Player contestantPlayer;
    
    public int numEnemies = 0;
    public int numKills = 0;
    
    // just subtract this from Time.time to get match time
    // useful if you add a 3, 2, 1, go at the beginning of the round.
    public float matchStartTime = 0;
    ...
    private float lastSpawnedGunTime = 0;
    public float gunSpawnDelay = 3;
    public float gunSpawnDistance = 100;
    ...
    void Update()
    {
        if (Time.time > lastSpawnedEnemyTime + enemySpawnDelay)
        {
            if(enemy != null)
                Instantiate(enemy, Random.insideUnitSphere * enemySpawnDistance, Random.rotation);

            lastSpawnedEnemyTime = Time.time;
        }
        
        if (Time.time > lastSpawnedGunTime + gunSpawnDelay)
        {
            int r = Random.Range(0, guns.Length);
            
            Instantiate(guns[r], Random.insideUnitSphere * gunSpawnDistance, Random.rotation);
 
            lastSpawnedGunTime = Time.time;
            
            print("spawned " + guns[r].name);
        }
    ...
    public void ContestantDies()
    {
        pressRToRestart.SetActive(true);
        endScreen.SetActive(true);

        kills.text = numKills.ToString();
        timeSurvived.text = (Time.time - matchStartTime).ToString();
