...
public class FirstPersonMovement : MonoBehaviour {
    /// <summary>
    /// Same FP Movement script from Sipoga but we will add jetpack
    /// 
    /// </summary>
	...
	private float jetpackSpeed = 0;
	public float jetpackPower = 1;
	public float jetpackResetRate = 0.1f;
	...
	void FixedUpdate() {
        ...
        if (CanMove)
        {
			...
			if (Grounded && (((SlideCounter > 2) && SlideWhenOverSlopeLimit) || (SlideOnTaggedObjects && _hit.collider.tag == "Slide")))
			{
				...
				Vector3 slideMoveDirection = new Vector3(hitNormal.x, -hitNormal.y, hitNormal.z);
			...
			// Add jetpack here
			if (canJetpack && Input.GetButton("Jetpack")) // maybe it can be left alt
			{
				jetpackSpeed += jetpackPower;
			}
			else
			{
				jetpackPower = Mathf.Lerp(jetpackPower, 0, jetpackResetRate);
			}
	        
			_moveDir.y += jetpackSpeed;
